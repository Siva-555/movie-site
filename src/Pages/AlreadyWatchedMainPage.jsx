    ...
    try{
      ...
      if(respFromIndexDB.bool && allRecords?.length>0){
        ...
        allRecords.forEach((ele)=>{
          if(ele?.type === "movie" && ele?.alreadyWatched === 1 && ele?.recordId) movieRecordsDB.push(ele);
          else if(ele?.type === "tvshow" && ele?.alreadyWatched === 1 && ele?.recordId) tvshowRecordsDB.push(ele);
        })
      ...
      <AnimatePresence mode="wait">
        {
          ...
          <EmptyMedia className="" cardKey="already-watched-empty" icon={<Film />} title="Looks like you haven’t watched anything yet!" subTitle="Watch something awesome and mark it as watched!" />
        }
      </AnimatePresence>
      {movies?.length>0 && <div>
...
export default AlreadyWatchedMainPage;
